...
- Revision: `dd5c7ef` on `master` — the 2026-08-16 harness investments merged via PR #15,
  with this report's CI observations added on top
...
| `.github/workflows/gate.yml` on PR #15 | both jobs green on a GitHub runner — `gate` in 21s, `end-to-end` in 3m57s with 7/7 checks. The first evidence any of these commands work off the machine they were written on |
...
| Execution | 4 enforced | `scripts/setup.py`, `scripts/check.py`, gateway and connector `build`/`start`/`stub`/`test`/`e2e`/`load` — all re-run green today, and both CI jobs ran green on a GitHub runner on PR #15 (gate 21s, end-to-end 3m57s, 7/7 checks), which is the first evidence the commands work off the machine they were written on | The Python interpreter name is still machine-dependent (`python` vs `py`) and resolved by the reader rather than the scripts; no timed cold-start from a clean clone |
| Feedback | 4 enforced | 240 passing unit tests; `run e2e` distinguishes success from failure per check and exits non-zero; the gate executes both suites and was proven to fail on a failing test; `run load` asserts the traffic split moves with the target; CI now reports both jobs on every pull request | No trend data — one CI run exists, so first-pass success and regression rates are still a single point rather than a series |
| Policy | 3 verifiable | `harness-validate`, the link check and both suites are pass/fail gates producing reproducible evidence; the gate's failure path is demonstrated, not assumed; the manifest now declares four policies, each naming the command that enforces it and the remediation for failing it; both gate jobs now run on every push and pull request | No lint or hook layer. CI runs but cannot be *required*: branch protection is unavailable on this repository's plan, so a red run does not block a merge — enforcement is mechanical in detection and human in effect |
...
| Policy | 2 | 3 | The same change turned the gate from a structural check into one carrying reproducible test evidence, and the failure path was proven rather than assumed. Not 4 even now that CI runs: the checks cannot be made required on this repository's plan, so a red run does not block a merge, and there is no lint or hook layer. |
...
| Execution | 3 | 4 | Both CI jobs ran green on a GitHub runner on PR #15 (gate 21s, end-to-end 3m57s). The commands are now proven reproducible off the machine they were written on, which is exactly what the baseline reading said would move this plane. |
...
| First-pass success | 1 of 1 CI runs green | PR #15: both jobs passed on the first attempt. One data point, not a rate |
| Regressions this period | 0 observed | all suites and the e2e run green locally and on CI at `dd5c7ef` |
...
1. **Make the checks blocking.** Both CI jobs run and both are green, but nothing requires
   them: branch protection needs a plan this private repository is not on, so a red run can
   still be merged — as a matter of fact rather than of policy, since PR #15 was merged by a
   human who had read the checks. Making the repository public, or upgrading, converts the
   last hop of enforcement from a habit into a mechanism and is what holds Policy at 3.
...
- Release and anything touching a remote remain **R0 inspect only**. One of the three
  blockers is cleared — a green CI run now exists, on PR #15 — but review outcomes are still
  unrecorded, there is no rollback path, and the checks cannot be made required, so a merge
  is still a human judgement that a human has to make.
